"use client";
import { UpdateUnread } from "@/actions/updateRead.action";
...
export default function SingleArticle({
  ...
  const formattedDate = summary.createdAt.toLocaleDateString("en-US", {
    month: "long",
    day: "numeric",
    year: "numeric",
  });
  ...
  useEffect(() => {
    const triggerUpdate = async () => {
      try {
        const result = await UpdateUnread(summary.id);
        if (result.success) {
    ...
    <>
      <div className="px-6 max-w-2xl mx-auto space-y-12">
        ...
        <section className="space-y-6">
          ...
          <img
            src={"https://picsum.photos/seed/detail/1200/800"}
            alt={summary.title}
        ...
        <section className="space-y-8">
          ...
          <div className="group">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h4 className="font-headline font-bold text-sm uppercase tracking-wide">
                Generative Precision
              </h4>
            </div>
            <p className="text-sm text-on-surface-variant leading-relaxed font-body">
              Algorithms optimize load-bearing paths that human engineers might
              overlook, saving tons of steel and concrete.
            </p>
          </div>
          <div className="group">
            <div className="flex items-center gap-3 mb-2">
            ...
            <p className="text-sm text-on-surface-variant leading-relaxed font-body">
              AI directly correlates structural integrity with carbon footprint
              reduction, enabling truly sustainable mega-cities.
            </p>
          </div>
        </div> */}
        </section>
        ...
        <section className="space-y-6">
          ...
          <ul className="space-y-6">
            {summary.commentAnalysis.map((analyzedComment, index) => (
        ...
        <section className="space-y-8">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant px-2 py-1 bg-surface-container-low rounded">
              {summary.topComments?.length || 0} Discussions
            </span>
          </div>
          <div className="space-y-10">
            {summary.topComments?.map((comment, i) => (
        ...
        <div className="pt-8 pb-12">
          <button
            onClick={() => router.push(summary.source)}
            className="w-full h-14 bg-primary text-on-primary font-headline font-bold text-sm rounded-lg flex items-center justify-center gap-2 hover:bg-primary-dim active:scale-[0.98] transition-all shadow-sm"
          >
